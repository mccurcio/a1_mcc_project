...
![Venn Diagrams of Bioinformatics Vs Data Science \label{venn_daigram}](../00-data/10-images/Bio-DS-Venn-diagram-42per.png)
...
![K-Nearest-Neighbor \label{knn}](../00-data/10-images/K-Nearest-Neighbor.png) [^18]
...
![K-Means \label{kmeans}](../00-data/10-images/k-means-2.png)[^112]

[^112]:https://www.slideshare.net/teofili/machine-learning-with-apache-hama/20-KMeans_clustering_20
...
To many predictive modeling is a panacea for all sorts of issues. Although this *may* be true now or in the future there are many hurdles which need to be overcome. Martin Jaggi[^113] has summarized four points which elucidate current problems in the field that need to be tackled.
...
[^113]:https://www.machinelearning.ai/machine-learning/4-big-challenges-in-machine-learning-ft-martin-jaggi-2/
...
It was decided early in the work that R[^114], RStudio[^115] and its machine learning library / framework `caret`[^116] would be used

[^114]:https://cran.r-project.org/

[^115]:https://rstudio.com/

[^116]:http://topepo.github.io/caret/index.html

#### Caret library for R

The R/caret library is attractive to use for many reasons. It currently allows some 238 machine learning models which have been written using disparate options and data structures. [^117] The utility of caret is that it organizes the input and output into a common format making the need for learning only one grammar and syntax.

[^117]:http://topepo.github.io/caret/available-models.html
...
The training section for caret can be broken into three parts, each atttempting to solve different questions. The most common questions that a researcher may have with regard to Predictive Modeling are:

##### How does one map the experimental space? 


##### How does one validate the results and assure reproducibility?
...
##### How does a researcher set up the parameters and options for each modeling attempt?

```
## Training caret-R code
set.seed(1000)

## Map a range of cost function values, C
...
- Using Testing data set Predict classes and generate CM.
