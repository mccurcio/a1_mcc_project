...
Therefore let us start by posing my first question, Is there a correlation between the data points which are outliers from principal component analysis (PCA) and several various types of predictive modeling. The reason for investigation was several fold. I was interested in finding if PCA would provide information on the false-positives and false-negatives that were an inevitable part of model building and optimization. I have learned from other Data Scientists working in industry that PCA is one of the first steps along with common Exploratory Data Analysis tools. 

PCA is an extremely useful tool but the results that it provides may not be immediately intuitive to the lay person. Several ideas that struck this researcher were regarding some of the advantages and dis-advantages for using PCA. The advantages are numerable and include dimesionality reduction, filter out noise inherent in the data and it may preserve the gloabl structue of the data.  This last point become the starting poinnt for my investigation. Does the global and graphical structure of the data produced in the first two principal components provide insight into how the predicttive models of Logistic Regressioon, Neural Networks which utilize autoencoders, Support Vector Machines and a Random Foresst approach.  In essence, is the tool PCA sufficiently different from the applied mathematic tools of more advanced approaches.
