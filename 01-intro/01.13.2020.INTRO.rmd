...
The term ‘Predictive Modeling’ should bring to mind work in the computer science field also called Machine Learning (ML), Artificial Intelligence (AI), Data Mining, Knowledge discovery in databases (KDD), Machine Intelligence, and possibly even encompassing Big Data as well.
...
Therefore let us start by posing a question, Is there a correlation between the data points which are outliers from principal component analysis (PCA) and several various types of predictive modeling. The reason for investigation was several fold. I was interested in finding if PCA would provide information on the false-positives and false-negatives that were an inevitable part of model building and optimization. I have learned from other Data Scientists working in industry that PCA is one of the first steps along with common Exploratory Data Analysis tools. 

PCA is an extremely useful tool but the results that it provides may not be immediately intuitive to the lay person. Several ideas that struck this researcher were regarding some of the advantages and dis-advantages for using PCA. The advantages are numerable and include dimensionality reduction, filter out noise inherent in the data and it may preserve the global structure of the data.  This last point become the starting point for my investigation. Does the global and graphical structure of the data produced in the first two principal components provide insight into how the predictive models of Logistic Regression, Neural Networks which utilize auto-encoders, Support Vector Machines and a Random Forest approach.  In essence, is PCA sufficiently similar to the applied mathematics tools of more advanced approaches.

The data for this study was derived from the Uniprot database. From the Uniprot database was queried for two protein groups. The first group was Myoglobin and the second was a control group comprised of human proteins not related to Hemoglobin or Myoglobin. There have been a group of papers that are striving to classify types of proteins by their amino acid structure alone. The simplest classification procedures involve simply using the percent amino acid composition.  This is calculated by using the count of a amino acid over the total number.

Percent Amino Acid Composition:

$$\%AAC ~=~ \frac{N_{Amino~Acid~X}}{Total ~ N ~ of ~ AA}$$
...
Regarding the machine learning tools that were used during this work
...
Given a training set TRAINING SET of N examples $(x_1, y_1), (x_2 , y_2),  . . .  (x_N , y_N)$,  we have both inputs and outputs, So Y can be either a real number for finding regression lines or a category for classification.
