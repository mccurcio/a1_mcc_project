...
#### Table 5.1: Organisms Vs Number of Neurons In Each ([Wikipedia](https://en.wikipedia.org/wiki/List_of_animals_by_number_of_neurons)) {-}
...
```{r fig.width=4, echo=FALSE, fig.align="center", fig.cap='Basic Neuron Types and S.E.M. Image'}
img <- readJPEG("../00-data/10-images/basicneurontypes.jpg")
grid.raster(img)
```
[^54]
...
```{r fig.width=4, echo=FALSE, fig.align="center", fig.cap="Two Neuron System (Image From The Public Domain)"}
img <- readPNG("../00-data/10-images/two.neuron.system.3.png")
grid.raster(img)
```
...
```{r fig.width=3, fig.height=1.2 ,fig.cap="Goal of a Tesla Neural Networks is to generate the correct repsonses for its environment.", fig.align="center", echo=FALSE}
img <- readPNG("../00-data/10-images/nn.black.box.png")
grid.raster(img)
```
...
```{r fig.width=4, fig.cap="One Neuron Schema", fig.asp="125%", fig.align="center", echo=FALSE}
img <- readPNG("../00-data/10-images/one.neuron.schema.2.png")
grid.raster(img)
```
...
\begin{equation} 
  \large \hat {Z}^{(1)} = f \left( \sum_{i=0}^n x_i w_i - T\right) = \{0, 1\}
\end{equation}
...
\begin{equation} 
  \large \hat Z^{(2)} ~=~ tanh(x) = \frac{1 - e^{-{\alpha}}}{1 + e^{-{\alpha}}} ~~~:~~~ \large where ~~~ \large \alpha = \sum_{i=1}^n x_i w_i - T
\end{equation}

\begin{equation} 
  \large \hat Z^{(3)} ~=~ sigmoid(x) ~=~ \frac{1}{1 + e^{-{\alpha}}}
\end{equation}

\begin{equation} 
  \large \hat Z^{(4)} ~=~ Hard ~ Tanh (x) ~=~ \large \left\{ \begin{array}{rcl} 1 &  x > 1 \\ x & -1 \leq x \leq 1 \\ -1 & x < -1 \end{array}\right.
\end{equation}
...
\begin{equation} 
  \large \hat Z^{(5)} ~=~ \large ReLU ~= \begin{cases} x \geqq 0 ~~~~y = x\\ x < 0 ~~~~y = 0 \end{cases}
\end{equation}
...
\begin{equation} 
  \large \hat Z^{(6)} ~=~ Softmax(x) = \frac {e^{\alpha_i}}{\sum_{i=1}^n e^{\alpha_i}}
\end{equation}
...
```{r fig.width=5, fig.cap="A Two Neuron System", fig.align = "center", echo=FALSE}
img <- readPNG("../00-data/10-images/two.neuron.system.png")
grid.raster(img)
```

### Feed-Forward In A Two Neuron Network ($X$ $\Longrightarrow$ $Z$)
...
### Back-propagation ($Z$ $\Longrightarrow$ $X$)
...
In the two neuron system, our first step is to generate an error or performance (Perf) function to minimize. If we call $d$ our desired value, we can minimize the square error, a common choice.[^510]

[^510]:Ivan N. da Silva, Danilo H. Spatti, Rogerio A. Flauzino, Luisa H. B. Liboni, Silas F. dos Reis Alves, Artificial Neural Networks: A Practical Course, DOI 10.1007/978-3-319-43162-8, 2017
...
```{r fig.height=1, fig.cap="A Two Neuron System", fig.align = "center", echo=FALSE}
img <- readPNG("../00-data/10-images/two.neuron.system.noframe.png")
grid.raster(img)
```
...
Step #4: Performance
\begin{equation}
  \frac{\delta Perf}{\delta z} ~~=~~ \frac{\delta \left\{ \frac{1}{2} \large \| \overrightarrow{d} - \overrightarrow{z} \|^2 \right\}} {\delta z} ~~=~~ \mathbf{\overrightarrow{d} - \overrightarrow{z}}
\end{equation}
...
My next investigation is into a class of NN called Auto-Encoders. Auto-Encoders (AE) use the same basic perceptron architecture as described earlier. In other words, they have the Summation and Activation functions and are computed using Feed-Forward and Error Back-Propagation. The key to building an auto-encoder is that there is a information bottleneck in the hidden layers of the NN.

```{r fig.width=3, echo=FALSE, fig.align="center", fig.cap="Generalized Auto-Encoder Schema With Bottleneck"}
img <- readPNG("../00-data/10-images/nn.ae.2.14.2020.png")
grid.raster(img)
```

To be more specific the AE are broken into two modes. Encoding and decoding are the two principle modes where information is transferred from input to output. 

In a AE system it is possible to have any number of inputs and outputs. In my case there are 20 AA that contain information regarding which polypeptides are of class 1 or 2.
