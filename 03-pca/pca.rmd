# Principle Component Analysis of Myoglobin Vs. Control
...
This chapter describes the use, intuition and a functional understanding when using PCA on the `c_m_TRANSFORMED.csv` myoglobin/control dataset. Principle Component Analysis (PCA) is very popular and commonly used during the Exploratory Data Analysis (EDA) phase to provide information on the proportions of variance found within a dataset. 

>The major goal of principal components analysis is to reveal hidden structure in a dataset. In so doing, we may be able to;[^1]
>
...
[^1]:Principal Components Analysis:A How-To Manual for R, Emily Mankin, http://people.tamu.edu/~alawing/materials/ESSM689/pca.pdf

---

## Finding the Covariance Matrix[^20]

[^20]:Trevor Hastie, Robert Tibshirani, Jerome Friedman, 'The Elements of Statistical Learning; Data Mining, Inference, and Prediction', Second Edition, 2009, Springer, DOI:10.1007/978-0-387-84858-7

[Covariance](http://mathworld.wolfram.com/Covariance.html) provides a measure of the strength of the correlation between two or more sets of random variates. 
...
Determining the covariance using matrix, $\large M$ using linear algebra notation is;[^15]

[^15]:http://mathworld.wolfram.com/Covariance.html
...
PCA is linked to the singular value decomposition (SVD) used when determining eigenvectors and eigenvalues.

>Singular value decomposition says that every n x p matrix can be written as the product of three matrices: $A = U \Sigma V^T$ where:[^3]
>
...
[^2]:Brian Everitt, Torsten Hothorn, An Introduction to Applied Multivariate Analysis with R, 2011, Springer, DOI:10.1007/978-1-4419-9650-3
...
### Two-dimensional example of PCA from random data: {-}
```{r echo=FALSE}
set.seed(1000)
x = seq(1, 4, 0.05)
...
NOTE: If you would like to investigate this phenomenon, the code below will generate a NO frills graphical plot.
...
xy_covar <- cov(cbind(x, y))
xy_eigen <- eigen(xy_covar)
...
There are many R-packages which will carry out the steps for PCA all behind the 'scenes' but giving no greater understanding for beginners. For example, `stats::prcomp`, `stats::princomp` are most commonly used. However, there are *several dozen* packages. A key word search for 'PCA' at [R-cran](https://cran.r-project.org/web/packages/available_packages_by_name.html) provides 78 matches, as of November, 2019.

## Data centering / scaling / normalization

While determining the variance of your dataset it should be clear that the order of magnitude of your data features matters greatly. The reasons for this should be clear that if one axis is in 1,000's  while the second axis is between 1 and 10, the larger scale will obviously have a greater variance. 

There are four common methods for **scaling** data:

No.|Scaling Method | Formula
---|---------------|--------------
1 | Centering | $f(x) ~=~ \large x - \bar x$
2 | Scaling between [0, 1] | $f(x) ~=~ \Large \frac {x - min(x)} {max(x) - min(x)}$
3 | Scaling between [a, b] | $f(x) ~=~ \large (b - a)* \Large \frac {x - min(x)} {max(x) - min(x)} + a$
4 | Normalizing | $f(x) ~=~ \Large \frac {x - mean(x)} {\sigma_x}$
...
#### Number of Outliers from PC-1 {-}

Rule Set:  

1. Outlier_1: c_m_20_PCA$x[, 1] > 5  

2. Outlier_2: c_m_20_PCA$x[, 1] < -5
```{r echo=FALSE}
...
3. Outlier_3: c_m_20_PCA$x[, 2] > 3.5  

4. Outlier_4: c_m_20_PCA$x[, 2] < -3.5  
```{r echo=FALSE}
outliers_PC2 <- which((c_m_20_PCA$x[, 2] > 3.5) | (c_m_20_PCA$x[, 2] < -3.5))
length(outliers_PC2)
```

#### List of all outliers found using rule set 1 through 4 {-}
```{r echo=FALSE}
...
Now it is important to remember and understand that this list of "total_pca_1_2_outliers" includes BOTH negative and positive controls. The delineation point for the controls is:

Controls = {1, ..., 1217}
Positive samples = {1218, ..., 2341}

- Keep outliers that are in the positive samples group, {1218, ..., 2341}.
- Produce data frame of Positive outliers with the percent amino acid compositions.
```{r eval=FALSE, include=FALSE}
...
The first of the two plots which I which is the scree plot.[^5] The scree plot is an ranked list of the eigenvalues plotted against its own principal components. An eigenvalue score of one is thought to provide a comparable amount of information as a single variable un-transformed by PCA.[^find]

[^5]:Cattell, Raymond (1966). "The scree test for the number of factors". Multivariate Behavioral Research. 1 (2): 245–76. doi:10.1207/s15327906mbr0102_10. PMID 26828106
...
Here again there are several criteria regarding how best to use the information from the is plot.  The first of which is Cattell's heuristic. Cattell advises to use the principal component that is above the elbow of the curve. The second heuristic is keeping the total number of factors that best explains 80%-95% of the variance.  There is no hard fast rule at this time. Whereas some researchers only use the first three factors or none at all.[^35]  

[^35]:Nicole Radzill, personal communication.
...
[^30]:https://docs.displayr.com/wiki/Kaiser_Rule#cite_note-1
