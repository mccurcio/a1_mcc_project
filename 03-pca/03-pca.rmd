...
abline(v = 2.76, col = "green")
```

| Graphic                 | Range (Green lines) | Differences |
| :---------------------- | :-----------------: | :---------: |
| Raw Data (Left)         |     1 <= x <= 4     |   3 units   |
| Trasformed Data (Right) | -2.45 <= x <= 2.76  | 5.21 units  |
...
Although the differences is between all three histograms is minor or at least on the same order of magnitude anyone *should* be sufficient to use. However I chose to use the *Normalized* dataset.
...
write.table(total_pca_1_2_outliers,
            file = "../00-data/03-outliers/pca_outliers.csv",
            row.names = FALSE,
            na = "",
            col.names = "rowNum",
            sep = ",")
...
Principal Component Analysis is very popular and an excellent choice to include during Exploratory Data. Analysis.  One objective for using PCA is to filter noise from the dataset used and in turn increase any signal or to sufficiently delineate observations from each other. In fact, one can see that there are five colored groups outisde the main body of observations. These five colored groups are labeled A-E.  It will be intereesting to see if any one of these grooups will be present in the group of false-positives and false-negatives in any of the mcahine learning models. Due the placement of each group in different areas of the Biplot we should be able to glean information of the percent amino acid compositions of the observations.

### Outliers derived from PC1 Vs PC2 {-}

The table below shows outliers when the first and second principal component are graphed with respect to the observation numbers. It will be very interesting to learn if there is a correlation between the outliers found by PCA are similar ot the false-positives and false-negatives from each of the machine learning models.
