...
The first step for calculating PCA is to determine the Covariance matrix. Covariance provides a measure of the strength of the correlation between two or more sets of random variates.[^32]
...
This simplified formula is to determine Covariance for a 2 dimensional system. Where $N$ is the number of observations, $\bar x$ is the mean of the independent varible, $\bar y$ is the mean of the dependent varialbe.
...
The first of the two plots which I which is the scree plot.[^310] The scree plot is a ranked list of the eigenvalues plotted against its own principal components. An eigenvalue score of one is thought to provide a comparable amount of information as a single variable un-transformed by PCA.[^FIND]
...
### Cumulative Proportion of Variance Vs Principal Components plot {-}
