...
>"Support Vector Machines should be in the tool bag of every civilized person." - Patrick Winston [^61]
...
Support Vector Machine (SVM) learning is a supervised learning technique that may be used as a binary classification system or to find regression formula. Support vector machines are a “maximum margin classifier. SVM finds the separating hyperplane with the maximum margin to its closest data points.” [^62]
...
In an imaginary biochemistry laboratory, researchers discover a novel enzyme found in different tissues throughout the human body. Biochemists purify the enzyme from several cadavers and several different tissue samples. There is literature that shows this newly found enzyme has two isozymes, 1) Alpha has a high enzyme activity and 2) Gamma has a lower enzyme activity. It know seems like a simple task to learn which isozyme is in which tissue samples. Therefore measuring the enzyme activities by using an assay with the production of a fluorescent compound would be appropriate.
...
You plot the enzyme activities found in the lab. In test #1, it is easy to see that the isozymes have distinct activities.  In this case, the isozymes can separated by activity alone. In this first figure, it is said the data is "linearly separable". [^63] To state the obvious, the two groups are linearly separable by the fact that a single straight line can partition the data.
...
Incidentally, in Patrick Winston's lecture on SVM,  he calls SVM the "widest street approach". Why does he use this term? There are many possible streets which can be traced BUT the goal is to find the *widest street*. This *widest street* leads to the best separation.

However, in test #1, we can see that linearly separable could imply an infinite number of lines with varying slopes that separate the groups. But this is where adding the idea or intuition that the optimal decision booundary, i.e. the widest highway, is advantageous because it leads us to a wider decision boundary and a higher ability to generalize.  In fact, "an optimal hyperplane is here defined as the linear decision function with maximal margin between the vectors of the two classes."[^64]

What does linearly separable data have to do with a higher ability to generlize in machine learning? One explanation harkens back to first semester statistics classes. Remember the discussion on Student's t-test and analysis of variance testing (ANOVA)?  
...
knitr::include_graphics("../00-data/10-images/p-values_schematic.jpg")
```
...
[^64]:C. Cortes, V. Vapnik, Machine Learning, 20, 273-297, 1995
...
plot(x = x1, y,
     main = "Enzyme Activities, Situation #2",
...
In the case above, the activities overlap hence determining which isozyme is Alpha or Gamma is more difficult. In 1995, C. Cortes and V. Vapnik introduced the mathematics and ideas for "Soft Margins" or non-separable training data.[^64] 
