...
In an imaginary biochemistry laboratory, researchers find a novel enzyme found in different tissues throughout the human body. Biochemists purify the enzyme from several cadavers and several different tissue samples. There is literature that shows this newly found enzyme has two isozymes, Alpha has a high activity and Gamma has a lower activity. It know seems like a simple task to learn which isozyme is in which tissue samples. Therefore investigate the enzyme activity by looking for the production of a fluorescent compound. 
...
Incidentally, in Patrick Winston's lecture on SVM,  he calls SVM the "widest street approach". There are many possible streets which can be traced BUT the goal is to find the *widest street*. This *widest street* leads to the best separation.

However, in the figure above, we can see that linearly separable could imply an infinite number of lines with varying slopes that separate the groups. But this is where adding the idea or intuition that the optimal decision booundary, i.e. the widest highway, is advantageous because it leads us to a wider decision boundary and a higher ability to generalize. In fact, "an optimal hyperplane is here defined as the linear decision function with maximal margin between the vectors of the two classes."[^64]
