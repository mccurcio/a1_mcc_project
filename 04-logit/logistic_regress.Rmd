...
If you have categorical data, how can one have a Y-values less than 0 or greater than 1. Yet, a linear regression would output such Y-values if we were to improperly use it here.

A better way would be to generate a curve where the Y value only varies between {0,1} and has a sloping curve that runs between them. The answer is a curve that follows

The alternative is a line that connects the Y-values 0 and 1 with a slope and values where the Y-values do not go beyond 0 and 1. 
