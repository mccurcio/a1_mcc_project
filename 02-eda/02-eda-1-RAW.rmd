...
1. Do the independent variables of this study have large skewed distributions?
    + If skew values greater than 2.0 are found, can a transformation be used for normalization?
    + If so, what transformation should be used?

2. Can **Feature Selection** be used and which procedures are appropriate?
    + Can the Random Forest technique known as Boruta[^2], be used for feature importance or reduction?
    + Will coefficients of correlation (R) find collinearity and reduce the number of features?
    + Will principle component analysis (PCA) be useful in finding hidden structures of patterns?
    + Can PCA be used successfully for Feature Selection?
...
3. What is the structure of the data?
    + Is the data representative of the entire experimental space?
    + Is missing data an issue?
    + Does the data have certain biases either known or unknown?
    + What relationships do we expect from these variables?[^3]
...
### Count RAW polypeptides by group
...
* How to reduce features given high correlation (|R| >= 0.75)
    ...
    + If the correlation plot produced of any two variables is greater than or equal to (|R| >= 0.75) then we could consider feature elimination. This interesting heuristic approach would be used for determining which feature to eliminate.[^12]
    + Determine if the two predictors associated with the largest absolute pairwise correlation (R > |0.75|), call them predictors A and B.
