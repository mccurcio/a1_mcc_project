...
## `svmRadial`: Model Summary
```{r}
model_obj
```

## `svmRadial`: Predict new samples (Class_test)
```{r, cache=TRUE}
...
```{r}
fp_fn_svm_Radial <- model_obj %>% pluck("pred") %>% dplyr::filter(obs != pred)

# Write CSV in R
write.table(fp_fn_svm_Radial, 
            file = "../20-outliers/fp_fn_svm_Radial.csv", 
            row.names = FALSE, 
            na = "", 
            col.names = TRUE, 
            sep=",")
