# Logistic Regression Using `Caret` & `Mass`

## Introduction

```{r message=FALSE, warning=FALSE}
## Load Libraries
Libraries = c("doMC", "knitr", "readr", "tidyverse", "caret", "MASS")
...
## `polr_Logistic`: Model Summary
```{r}
model_obj
```

## `polr_Logistic`: Predict new samples (Class_test)
```{r, cache=TRUE}
...
## `polr_Logistic`: Confusion Matrix
```{r}
...
```{r}
fp_fn_polr_logit <- model_obj %>% pluck("pred") %>% dplyr::filter(obs != pred)

# Write CSV in R
write.table(fp_fn_polr_logit, 
            file = "../20-outliers/fp_fn_polr_logit.csv", 
            row.names = FALSE, 
            na = "", 
            col.names = TRUE, 
            sep=",")
