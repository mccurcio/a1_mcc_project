...
```{r 52,fig.height=2,fig.width=4,echo=FALSE,fig.align="center",fig.cap='Basic Neuron Types and S.E.M. Image'}
...
```{r 53,fig.height=1.5,fig.width=4,echo=FALSE,fig.align="center",fig.cap="Two Neuron System (Image From The Public Domain)"}
...
```{r 54,fig.width=3,fig.height=1.5,fig.cap="Goal of a Tesla Neural Networks is to generate the correct responses for its environment.",fig.align="center",echo=FALSE}
...
```{r 55,fig.height=2,fig.width=4,fig.cap="One Neuron Schema",fig.asp="125%",fig.align="center",echo=FALSE}
...
```{r 58,fig.height=2,fig.width=5,fig.cap="A Two Neuron System",fig.align = "center",echo=FALSE}
...
```{r 59,fig.height=1,fig.cap="A Two Neuron System",fig.align = "center",echo=FALSE}
img <- readPNG("./00-data/10-images/two.neuron.system.noframe.png")
grid.raster(img)
```

Using the chain-rule, and figure 6, *Two Neuron System* as a guide, we can backwards to derive the formulas for error back-propagation. We find:
...
Step #4: Performance
\begin{equation}
  \frac{\delta Perf}{\delta z} ~~=~~ \frac{\delta \left\{ \frac{1}{2} \| \overrightarrow{d} - \overrightarrow{z} \|^2 \right\}} {\delta z} ~~=~~ \mathbf{d - z}}
\end{equation}
...
```{r 514, include=FALSE, cache=TRUE}
set.seed(1000)
...
Accuracy was the primary criteria used to select the optimal model. There were 5 models tested. The neural network was configured with 20 inputs (one for each amino acid), one hidden layer and one output. The hidden layer was tested with either 10, 12, 14, 16, 18, 20 neurons and a array of different decays (1, 0.1, 0.01, 0.001). The caret software has a tuning parameter named `tuneGrid` that allows users to `expand` a set of arrays to a matrix of combinations to be tested. Therefore 20 models were tested with the training data set and the best values were size = 20 and decay = 0.1.

At this time, the author is not aware of any hueristic that gives the proper number of hidden layers and the proper number of neurons in each layer therefore one must search the experimental space for an optimized  configuration. 


 
To this researcher this does make intuitive sense. If there is no redundancy in the features then 20 there must be at least 20 decision boundaries in order to classify a binary classification system. 
