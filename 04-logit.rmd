...
For individuals who have studied biology or biochemistry, logistic regression may be familiar as dose-response curves[^46], enzyme kinetic curves[^47], median lethal dose curve (LD-50)[^48] or even an exponential growth curve, given  a set of limited resources[^49]. 
...
Logistic regression (Logit) derives its name from its similarity to linear regression, as we shall see below. The input or independent variable(s) for Logit, may be a matrix ($X^{m ~x~ n}$) in the set of real numbers, ($X ~ \in ~ \Re$). While, the output of a Logistic Regression is represented by {0, 1}. Therefore, the output of Logit is NOT IN the set of real numbers ($Y ~ \notin ~ \Re$),
