...
If we are to discuss Neural Networks, we could start by considering our current gold standard, a number of biologial computing systems and the number of neurons each has. By investigating the number of neurons in organisms from along earth's phylogenetic or evolutionary tree we might get a better idea of the type of computing power computer scientists might need to surpass anyone of these creatures. See table 1.
...
$$\large Z = tanh(x) \equiv \frac{1 - e^{-{\alpha}}}{1 + e^{-{\alpha}}} ~~~~or~~~~\large Z = sigmoid(x) = \frac{1}{1 + e^{-{\alpha}}}$$

$$where ~~~ \alpha = \sum_{i=1}^n x_i w_i - T$$
...
plot(x, y_tanh, 
     type = "l",
     main = "Hyperbolic Tangent Profile",
     ylab = "tanh(x)")
abline(h = 0, v = 0, col = "#F24F4F")
plot(x, y_sigmoid,
     type = "l",
     ylim = c(-1,1),
     main = "Logistic / Sigmoidal Profile",
     ylab = "sigmoid(x)")
abline(h = 0, v = 0, col = "#F24F4F")
```
...
$$\large ReLU ~= \begin{cases} x \geqq ~~~~y = x\\ x < ~~~~y = 0 \end{cases}$$
...
[^6]:Josh Patterson, Adam Gibson, Deep Learning; A Practitioner’s Approach, 2017, O'Rreilly
...
$$\large Softmax(x) = \frac {e^{\alpha_i}}{\sum_{i=1}^n e^{\alpha_i}} ~~~~:~~~~ where ~~~ \alpha = X^T \cdot W + b$$
